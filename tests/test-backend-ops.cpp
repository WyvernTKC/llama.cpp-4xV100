...
struct test_rwkv7_output : public test_case {
    ...
    // time_mix_ln, time_mix_ln_b and time_mix_r_k are model weights in the real
    // graph. Allocating them in the weight context keeps the reshape of r_k a
    // view of a constant tensor, which is what ggml_can_fuse_subgraph requires
    // before the CUDA backend will elide the intermediate nodes.
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    test_cases.emplace_back(new test_flash_attn_ext(512, 512, 1, { 8, 1}, 8192, 66, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false,  512));
    test_cases.emplace_back(new test_flash_attn_ext(512, 512, 1, { 8, 1}, 16384, 64, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, false, 1024));
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
 ...
 // High resolution output (large NPQ)
    ...
    // Ornith-1.5-35B-A3B (qwen35moe): 256 experts, top-8, n_ff_exp 512, n_embd 2048, ubatch 2048.
    // One device's share of the MoE under the 4-way -sm tensor split: gate/up loses output
    //   width, down loses reduction depth. ~64 tokens are routed to each expert.
    for (ggml_type type_a : {GGML_TYPE_Q8_0}) {
        test_cases.emplace_back(new test_mul_mat_id(type_a, GGML_TYPE_F32, 256, 8, false,  128, 2048, 2048)); // gate/up
        test_cases.emplace_back(new test_mul_mat_id(type_a, GGML_TYPE_F32, 256, 8, false, 2048, 2048,  128)); // down
    }
    ...
    // deepseek4 has key_length == value_length == 512
    test_cases.emplace_back(new test_flash_attn_ext(512, 512, 8, {8, 1},  8192,   1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16));
    test_cases.emplace_back(new test_flash_attn_ext(512, 512, 8, {8, 1},  8192, 512, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16));
    test_cases.emplace_back(new test_flash_attn_ext(512, 512, 8, {8, 1}, 16384, 512, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16));

    // bailingmoe3 / deepseek MLA has key_length 576, value_length 512, V is a view of K
    test_cases.emplace_back(new test_flash_attn_ext(576, 512, 1, {16, 1},  8192, 512, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, true));
    test_cases.emplace_back(new test_flash_attn_ext(576, 512, 1, {16, 1}, 16384, 512, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16, GGML_TYPE_F16, {0, 1, 2, 3}, true, true));
