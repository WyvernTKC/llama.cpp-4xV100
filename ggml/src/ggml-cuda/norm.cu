...
static __global__ void rms_norm_f32(const float * x,
    ...
    static_assert(!do_scale || !do_multiply, "fusing a scalar scale is only supported without multiplying");
    ...
    for (int col = tid; col < ncols; col += block_size) {
        if constexpr (do_multiply && do_add) {
            ...
            dst[col] = scale * x[col] * post_scale + post_bias;
        } else {
            dst[col] = scale * x[col];
        }
    }
...
// rms norm with a following scalar ggml_scale folded into the epilogue: dst = rms(x) * s + b
static void rms_norm_scale_f32_cuda(
        ...
        const float post_scale, const float post_bias, cudaStream_t stream) {
    const dim3 blocks_num(nrows, nchannels, nsamples);
    if (ncols < 1024) {
        const dim3 block_dims(256, 1, 1);
        const ggml_cuda_kernel_launch_params launch_params = {blocks_num, block_dims, block_dims.x > WARP_SIZE ? 32 * sizeof(float): 0, stream};
...
void ggml_cuda_op_rms_norm_fused_scale(ggml_backend_cuda_context & ctx, ggml_tensor * dst, ggml_tensor * scale_tensor) {
    const ggml_tensor * src0 = dst->src[0];
    const float * src0_d = (const float *) src0->data;
    ...
    float post_scale;
    float post_bias;
    memcpy(&post_scale, (const float *) scale_tensor->op_params + 0, sizeof(float));
    memcpy(&post_bias,  (const float *) scale_tensor->op_params + 1, sizeof(float));
    ...
    rms_norm_scale_f32_cuda(src0_d, dst_d, src0->ne[0], src0->ne[1], src0->ne[2], src0->ne[3],
                            s01, s02, s03, eps, post_scale, post_bias, stream);
}
