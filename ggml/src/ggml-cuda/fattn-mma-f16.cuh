...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps,
    bool use_logit_softcap, bool V_is_K_view, bool use_sparse, bool needs_fixup, bool is_fixup, bool last_iter, bool oob_check, int dvp,
...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps, bool use_logit_softcap, bool V_is_K_view, bool use_sparse, bool needs_fixup, bool is_fixup, int dvp>
...
#elif defined(AMD_MFMA_AVAILABLE)
    T_C_VKQ VKQ_C[                                     DV_acc/T_C_VKQ::J];
#elif defined(AMD_WMMA_AVAILABLE)
    ...
    if constexpr (ncols2 == 1 || use_sparse) {
        ...
        for (; kb0 < kb0_stop-1; ++kb0) {
            ...
            flash_attn_ext_f16_iter
                <DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, use_sparse, needs_fixup, is_fixup, last_iter, oob_check, dvp,
...
static __device__ __forceinline__ void flash_attn_ext_f16_process_tile_dv(Args ... args) {
    ...
    flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, use_sparse, needs_fixup, is_fixup, 0>(args ...);
    if constexpr (DV_split == 2) {
        flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, use_sparse, needs_fixup, is_fixup, 1>(args ...);
    }
}
...
static constexpr __host__ __device__ bool ggml_cuda_flash_attn_ext_mma_f16_may_use_sparse(
    ...
    // Volta's MMA kernel has no device code below 32 columns, so its sparse tile is 4 queries wide and
    // the index row is the union of the 4 queries' selected keys - the mask gather keeps each query exact
    return (DKQ == 512 && DV == 512 && ncols1 == 1 && ncols2 == 8) ||
           (DKQ == 512 && DV == 512 && ncols1 == 4 && ncols2 == 8) ||
...
static __global__ void flash_attn_ext_f16(
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        if (kb0_start == 0) {
            ...
            flash_attn_ext_f16_process_tile_dv<DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, use_sparse, needs_fixup, is_fixup>
...
void ggml_cuda_flash_attn_ext_mma_f16_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (logit_softcap == 0.0f) {
        ...
        if constexpr (ggml_cuda_flash_attn_ext_mma_f16_may_use_sparse(DKQ, DV, ncols1, ncols2)) {
            if (ggml_cuda_flash_attn_ext_mma_f16_shall_use_sparse(ctx, dst, ncols1)) {
