...
struct ggml_tensor * ggml_get_rows(
        ...
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    // the rows are converted to F32, except for I32 which is copied
    return ggml_get_rows_impl(ctx, a, b, a->type == GGML_TYPE_I32 ? a->type : GGML_TYPE_F32);
}

struct ggml_tensor * ggml_get_rows_same_type(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b) {
    return ggml_get_rows_impl(ctx, a, b, a->type);
}
