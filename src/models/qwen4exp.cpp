...
// a shared MTP draft has no embedding or head of its own and reads the target's
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // an MTP draft file holds only the NextN block, so the trunk tensors are absent
    ...
    for (int il = 0; il < n_layer_all; ++il) {
        ...
        // a self-contained draft carries its own embedding and head, a shared one borrows the target's
...
llama_model_qwen4exp::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // an MTP draft needs the hidden state of every token, so gather after publishing it
    ...
    if (cparams.embeddings_nextn) {
        ggml_tensor * h_nextn = ggml_cont_2d(ctx0, res_hc, n_embd*hc, res_hc->ne[2]);
...
// The draft head uses dense attention: it has no QSA path of its own.
llama_model_qwen4exp::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params) :
    ...
    GGML_ASSERT(hparams.n_layer_nextn == 1 && "QWEN4EXP MTP requires one NextN block");
    ...
    ggml_tensor * h_nextn = ggml_cont_2d(ctx0, res_hc, hc_dim, res_hc->ne[2]);
    cb(h_nextn, "h_nextn", -1);
    res->t_h_nextn = h_nextn;
    ggml_build_forward_expand(gf, h_nextn);
